...
def get_demand(PopVar, scenario, start, end, daily = False):
    """
    Parameters
    ----------
    PopVar : str
    ...
    """
    if daily:    
        dr = pd.date_range(pd.to_datetime(start, dayfirst=True), pd.to_datetime(end, dayfirst=True), freq = "d")
    else:
        dr = pd.date_range(pd.to_datetime(start, dayfirst=True), pd.to_datetime(end, dayfirst=True), freq = "ME")
    path = '../GW40_sim_engine-master/GW40_sim_engine-master/LongTermProjections/'
    ...
    if daily:
        ...
        ewm = ew.resample("ME").mean().interpolate()
...
def check_rates(demand_, rates_, wr, wr_ts_, restrictions, verbose = True, couple_compensation = False):
    if verbose:
...
def demand2well(demand, bwv, restrictions, how="day"):
    """
    ...
    """
    path = '../GW40_sim_engine-master/GW40_sim_engine-master/'
    wr = pd.read_csv(path+"asg_data/Wasserlinke/Wasserrechte ASG ab 2024.csv")
    wr.set_index("Brunnen ", inplace = True)
    wr.drop("BWV",axis = 0, inplace = True)
    wells = pd.read_csv(path+"asg_data/asg_well_rates/wells_asg_swt_17_21.csv")
    ...
    return rates, bwv, wr_ts_withRes


def getWellRates(PopVar, scenario, start, end, restrictions, bwv = None,  unit = "m^3/d"):
    ...
    if "BWV" in restrictions.keys():
        ...
        bwv_ts.loc[cond,"BWV"] = restrictions["BWV"]["rate"]
    
    
    demand_after_bwv = demand-bwv_ts.values#.sub(bwv_ts, axis = 1)
    
    bwv_ts = pd.DataFrame(bwv_ts, index=demand.index)
    well_rates, bwv, wr_ts = demand2well(demand_after_bwv, None , restrictions, how = "annual")
...
if __name__ == "__main__":
    ...
    restrictions = {"TB Altingen 3": {"rate": 0, "start": "01.05.2025", "end": "30.08.2025", "year": 2025},
                    "TB Breitenholz":{"rate": 0, "start": "01.05.2025", "end": "30.08.2025", "year": 2025},
                    # "TB Kiebingen 1":{"rate": 0, "start": "01.02.2025", "end": "30.04.2025", "year": 2025},
                    "TB Entringen 2":{"rate": 0, "start": "01.05.2025", "end": "30.08.2025", "year": 2025},
                    "TB Kiebingen 3":{"rate": 0, "start": "01.05.2025", "end": "30.08.2025", "year": 2025},
                    # "BWV": {"rate": 0, "start": "01.05.2025", "end": "30.08.2025", "year": 2025},
                    }
